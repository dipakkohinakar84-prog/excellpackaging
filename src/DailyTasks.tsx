...
const DailyTasks: React.FC<Props> = ({ loggedInUser }) => {
  ...
  const handleStatusChange = async (id: number, status: string) => {
    await supabase.from('daily_tasks').update({ status, updated_at: new Date().toISOString() }).eq('id', id);
    fetchTasks();
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-1.5 flex-wrap">
        {statuses.map(s => (
          <button key={s} onClick={() => setFilter(s)} className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${filter === s ? tabColors[s] : 'bg-gray-100 text-gray-600 border-transparent hover:bg-gray-200'}`}>{s}</button>
        ))}
      </div>
          ...
          {filtered.map(task => (
            <div key={task.id} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${priorityColors[task.priority]}`}>{task.priority}</span>
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-black border ${statusColors[task.status]}`}>{task.status}</span>
                  </div>
              ...
              <div className="flex gap-1.5 mt-3 pt-3 border-t border-gray-100">
                {['Pending', 'In Progress', 'Completed', 'Cancelled'].filter(s => s !== task.status).map(s => (
                  <button key={s} onClick={() => handleStatusChange(task.id, s)} className={`px-2.5 py-1 rounded-lg text-[10px] font-bold border transition-colors ${changeBtnColors[s]}`}>{s}</button>
                ))}
              </div>
            </div>
          ))}
